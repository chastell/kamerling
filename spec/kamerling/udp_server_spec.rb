...
module Kamerling describe UDPServer do
  describe '.new' do
    it 'sets up an UDP server on the given host and port' do
      UDPServer.new(host: '0.0.0.0', port: 2009).start
      UDPSocket.new.connect '0.0.0.0', 2009
    end

    it 'defaults to localhost' do
      UDPServer.new.addr.host.must_equal '127.0.0.1'
    end

    it 'defaults to random, unused ports' do
      s1, s2 = UDPServer.new.start, UDPServer.new.start
  ...
  describe '#start' do
    it 'listens on an UDP port and passes the received input to the handler' do
      server = UDPServer.new(handler: handler = fake(:handler)).start
  ...
  describe 'logging' do
    ...
    let(:server) { UDPServer.new host: '0.0.0.0', logger: logger }
